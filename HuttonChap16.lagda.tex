...
idTree : (t : Tree a) → (fmap id) t ≡ id t
idTree (Leaf x) = refl
idTree (Node tₗ tᵣ) =
  begin
    fmap id (Node tₗ tᵣ)
  ...
  ≡⟨ cong (λ x → Node x (fmap id tᵣ)) (idTree tₗ) ⟩
    Node (id tₗ) (fmap id tᵣ)
  ≡⟨ cong (Node (id tₗ)) (idTree tᵣ) ⟩
    Node (id tₗ) (id tᵣ)
  ≡⟨⟩ -- Apply and unapply id
    id (Node tₗ tᵣ)
  ∎
  
compTree : (t : Tree a) → (f : a → b) → (g : b → c)
  → fmap (g ∘ f) t ≡ (fmap g ∘ fmap f) t
compTree (Leaf x) f g = refl
compTree (Node tₗ tᵣ) f g =
  begin
    fmap (g ∘ f) (Node tₗ tᵣ)
  ≡⟨⟩ -- Apply fmap
    Node (fmap (g ∘ f) tₗ) (fmap (g ∘ f) tᵣ)
  ≡⟨ cong (λ x → Node x (fmap (g ∘ f) tᵣ)) (compTree tₗ f g) ⟩
    Node ((fmap g ∘ fmap f) tₗ) (fmap (g ∘ f) tᵣ)
  ≡⟨ cong (Node ((fmap g ∘ fmap f) tₗ)) (compTree tᵣ f g) ⟩
...
module LawfulFunctorTree where
  ...
  instance
    ...
    identity ⦃ isLawful ⦄ = idTree
    composition ⦃ isLawful ⦄ = compTree
\end{code}
...
\begin{code}
idMaybe : {a : Set} → (m : Maybe a) → (pure id <*> m) ≡ m
idMaybe Nothing =
  begin
    pure id <*> Nothing
  ≡⟨⟩ -- Apply pure and <*>
...
compMaybe (Just x) Nothing z =
  ...
  ≡⟨⟩ -- Unapply <*>
    Just x <*> (Nothing <*> z)
  ∎
compMaybe (Just x) (Just y) Nothing =
  refl -- Same kind of proof as above.
\end{code}

\begin{code}
compMaybe (Just x) (Just y) (Just z) =
  begin
...
module LawfulApplicative where
  ...
  instance
    ...
    identity ⦃ isLawful ⦄ = idMaybe
    composition ⦃ isLawful ⦄ = compMaybe
