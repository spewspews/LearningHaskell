...
module Exercise11 where
  ...
  comp'-def : (e : Expr) → (c : Code)
    → comp' e c ≡ CC.comp e c
  comp'-def (Val n) c =
    begin
      comp' (Val n) c
    ≡⟨ comp'-comp (Val n) c ⟩
  ...
  comp'-def (Add eₗ eᵣ) c =
    begin
      comp' (Add eₗ eᵣ) c
    ≡⟨ comp'-comp (Add eₗ eᵣ) c ⟩
      comp (Add eₗ eᵣ) ++ c
    ≡⟨⟩ -- Apply comp
      (comp eₗ ++ (comp eᵣ ++ ADD ∷ [])) ++ c
    ≡⟨ ++-assoc (comp eₗ) (comp eᵣ ++ ADD ∷ []) c ⟩
      comp eₗ ++ ((comp eᵣ ++ ADD ∷ []) ++ c)
    ≡⟨ cong (comp eₗ ++_) (++-assoc (comp eᵣ) (ADD ∷ []) c) ⟩
      comp eₗ ++ (comp eᵣ ++ (ADD ∷ [] ++ c))
    ≡⟨⟩ -- Apply ++
      comp eₗ ++ comp eᵣ ++ ADD ∷ c
    ≡⟨ cong (comp eₗ ++_) (sym (comp'-comp eᵣ (ADD ∷ c))) ⟩
      comp eₗ ++ comp' eᵣ (ADD ∷ c)
    ≡⟨ sym (comp'-comp eₗ (comp' eᵣ (ADD ∷ c))) ⟩
      comp' eₗ (comp' eᵣ (ADD ∷ c))
    ≡⟨ cong (comp' eₗ) (comp'-def eᵣ (ADD ∷ c)) ⟩
      comp' eₗ (CC.comp eᵣ (ADD ∷ c))
    ≡⟨ comp'-def eₗ (CC.comp eᵣ (ADD ∷ c)) ⟩
